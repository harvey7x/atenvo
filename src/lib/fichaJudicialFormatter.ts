...
function linhaRevisao(r: FichaRevisaoFmt): string {
  if (lim(r.descricaoLivre)) return lim(r.descricaoLivre);
  const tipo = r.tipo === 'agibank' ? 'AGIBANK' : r.tipo === 'outro' ? '' : r.tipo.toUpperCase();
  const banco = [lim(r.bancoCodigo), lim(r.bancoNome)].filter(Boolean).join(' - ');
  const cabeca = ['REV', tipo, banco].filter(Boolean).join(' ');
  const base = cabeca || 'REV';
