...
import { formataTelefoneBR, calculaIdade, hojeISOSaoPaulo } from './fichaJudicialNormalizers';

const DATA_FICHA = '2026-07-23'; // data fixa para o teste ser determinístico

/** Monta a ficha exatamente como o modal monta (parser → formatter). */
function gerar(bloco: string, gerente = 'Matheus', dataFicha = DATA_FICHA) {
  ...
  const texto = formatarFichaJudicial({
    ...
    telefone: formataTelefoneBR(r.telefone ?? ''), estadoCivil: '', email: r.email ?? '',
...
describe('Caso 3 — Maria Elisa (bloco real do Promosys)', () => {
  const { r, texto } = gerar(BLOCO_MARIA);
  ...
  it('telefone vem da seção TELEFONES', () => { expect(r.telefone).toBe('51999416800'); });
...
describe('Caso 1 — Nelci', () => {
  const { r, texto } = gerar(BLOCO_NELCI);
...
describe('Caso 2 — Linequer', () => {
  const { r, texto } = gerar(BLOCO_LINEQUER);
...
describe('anti-contaminação: bloco novo apaga o anterior', () => {
  it('reanalisar com outro bloco troca todos os campos', () => {
    const a = gerar(BLOCO_MARIA);
    const b = gerar(BLOCO_NELCI);
...
describe('DATA da ficha', () => {
  it('usa a data atual de America/Sao_Paulo', () => {
    const { r } = gerar(BLOCO_MARIA, 'Matheus', undefined as unknown as string);
