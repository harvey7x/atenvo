...
export function useWaCanais() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<WaCanal[]> => {
      ...
      const { data, error } = await supabase!
        .from('canais')
        .select('id, nome_interno, numero_conectado, status_integracao, provider, conectado_em, envio_restrito, entrega_status, entrega_erros_recentes, origem_tipo, gestor_id, fonte_aquisicao_id, campanha, observacao_comercial, gestor:usuarios(nome)')
        .eq('organizacao_id', currentOrg.id).eq('tipo', 'whatsapp').eq('provider', 'evolution')
        .neq('status_integracao', 'removido')
        .order('criado_em', { ascending: true });
      if (error) throw new Error(error.message);
      type Row = { id: string; nome_interno: string | null; numero_conectado: string | null; status_integracao: string; provider: string | null; conectado_em: string | null; envio_restrito: boolean | null; entrega_status: string | null; entrega_erros_recentes: number | null; origem_tipo: string | null; gestor_id: string | null; fonte_aquisicao_id: string | null; campanha: string | null; observacao_comercial: string | null; gestor: { nome: string } | { nome: string }[] | null };
      return ((data as Row[]) ?? []).map((r) => ({
        ...
        entregaStatus: (['ok', 'instavel', 'restrito'].includes(r.entrega_status ?? '') ? r.entrega_status : 'desconhecido') as WaCanal['entregaStatus'], entregaErrosRecentes: r.entrega_erros_recentes ?? 0,
