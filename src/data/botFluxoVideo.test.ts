...
describe('máquina de passos — abertura, SIM/NÃO, nome, CPF, resultado', () => {
  it('abertura: texto + VÍDEO com legenda + pergunta SIM/NÃO no mesmo burst', () => {
    const r = proximoPassoVideo(null, digitou('oi'), 0, 0, {}, COPY);
    if (r.acao !== 'enviar') throw new Error('esperava enviar');
    expect(r.telas).toHaveLength(4);   // 1 abertura + 1 vídeo + 2 pergunta
    expect(r.telas[0]).toEqual({ tipo: 'texto', corpo: COPY.abertura[0] });
    expect(r.telas[1]).toEqual({ tipo: 'video', url: COPY.video_url, caption: COPY.video_caption });
    expect(r.telas[2]).toEqual({ tipo: 'texto', corpo: COPY.pergunta_analise[0] });
    expect(r.telas[3]).toEqual({ tipo: 'texto', corpo: 'Responda *SIM* ou *NÃO*' });
    expect(r.passoNovo).toBe('aguardando_sim_nao');
  });

  it('sem video_url configurada, o vídeo é pulado (mídia nunca trava o funil)', () => {
  ...
  it('nome válido → salva e pede o CPF (2 balões, com a promessa de sigilo)', () => {
