...
- **Relatórios v2.1 + Modo Apresentação (Ordem de Redesenho, 2026-07-28 — DECISÃO DE PRODUTO registrada, precedente Agendamentos/Scripts)**: camada visual além do manual (funil do tráfego agregado+por conexão com as colunas existentes como etapas; ranking com pódio platina e métrica selecionável — bot/Não atribuído não competem; insights honestos com regra visível e AMOSTRA_MINIMA=8 declarada — abaixo dela, absoluto sem veredito; sparkline só onde a série diária existe: leadsSerie) + MODO APRESENTAÇÃO (takeover fullscreen em portal, --fx:1, 6 slides derivados dos MESMOS hooks/queryKeys — cache compartilhado, zero query nova; auto-avanço ~8s, clique avança BLOCO, ←/→/espaço/Esc, slide sem dados pulado, reduced-motion = manual estático). FORA por falta de sustentação: corte "qualificados pelo bot" (dados_qualificacao vive em bot_conversa_estado, acessível só por edge/service_role — sem hook para espelhar) e taxonomia de remarketing (canais RMKT identificáveis só por nome; o funil por conexão conta a história). A página de trabalho segue dado-calma; o cinema é um modo.
...
## Plano da próxima rodada (registrado 2026-07-28, ordem final da rodada)
Restante em sessões próprias: **Integrações → Kanban → WhatsApp**.
**Dashboard / Facebook / Relacionamento = ManutencaoV2** (página única honesta, sem prazo) até segunda ordem — rotas vivas, sidebar intacta (item de sidebar do Relacionamento entra na sessão própria dele).
**No corte:** a home "/" redireciona para **/v2/whatsapp** (como o v1 faz hoje) até o Dashboard existir; atualizar destino.ts (DESTINO_PADRAO_V2) nesse momento.
Cortina: SEMPRE em Relatórios (Adendo nº 5 revisado; cooldown e Replay-dev removidos).
...
**Sessão de correção crítica (2026-07-28, commit "v2: whatsapp — correção crítica e auditoria completa"):** RAIZ do "modal branco" = o Modal.tsx **v1** (usado por FichaJudicialModal + MediaComposer + ScriptSequenceModal) referencia tokens de tema claro (--surface/--ink/--line/--accent) inexistentes no .v2 → fallback branco. Por que o guard não pegou: ele só detecta REDECLARAÇÃO de classe em CSS v2, não componente v1 reusado cujo CSS v1 estiliza o DOM v2 sem oposição. CORRIGIDO: override `.v2 .atv-modal-overlay` redefine os tokens v1 para valores Platina (conserta chrome + inputs + fj-* de uma vez) + fio-de-luz no `.atv-modal` + `.atv-btn.primary`/`.danger` platina/rubro + textos `.fj-ind/.fj-alerta/.fj-erro` de contraste. GUARD REFORÇADO: allowlist ampliado (fjb/fj-/atv-) + 2º teste que varre TSX v2 por classe v1 de seletor-ÚNICO (aplica global) sem override v2 (pega "código v2 escreve classe v1"). Auditoria adversarial (6 finders Opus 4.8 + refutação): 4 confirmados corrigidos — resp-btn movido p/ lado INTERNO da bolha + overflow-x:hidden em .wa-msgs (era externo → corte/rolagem); Bolha ganhou estado "Carregando imagem/vídeo…" (o "indisponível" piscava durante a signed-URL async); demo do FichaJudicialModal REVERTIDO (o save batia em supabase null → erro cru; demo é estanque mas o modal real não tinha guarda demo — botão "Criar ficha" no demo agora só avisa). Baixas: opt-out fora do agregado ⚠ (chip único); "Finalizado" oculto quando situação já é terminal (Adendo 3) + seed da Cleusa usa oppStatus='perdido' (não etapa string); cr por tier (neutro=txt-3, não âmbar); falhou só p/ saída; envioSaude fallback current.canalId; currentOrg nas deps da mídia. DEFERIDO (declarado, NÃO é regressão desta rodada): a fila de 371 linhas re-renderiza a cada tecla (draft no mesmo componente) — fix recomendado (isolar composer / memoizar linhas) fica p/ passo de perf dedicado, para não arriscar staleness no inbox numa sessão de correção. DEPLOY: os commits são locais no worktree; se o dono revisa via preview CF Pages, precisa do branch empurrado/redeployado.

**Sessão de fidelidade (2026-07-28, commit "v2: whatsapp — fidelidade e auditoria completa"):** DIAGNÓSTICO da "luz no hover sumida" = as superfícies do WhatsApp nunca receberam `.spot` (não é colisão de merge — guard passa; o mockup pg-whatsapp tem painéis planos, mas o dono quis o glow do sistema). CORRIGIDO: (a) SPOTLIGHT — `.spot` em `.conv2`/`.ctx-topo`/`.ctx-b` com o glow base (.10×--fx, igual Contatos) e lift suprimido (listas/painéis não saltam); (b) RECOLHER do painel de contexto — `.wa-ctx` sempre montado + classe `.recolhido` (transition width .28s, a conversa reflowa via flex) + `.wa-ctx-inner` fixo 266px + persistência sessionStorage 'atenvo-wa-ctx' + `inert` nos filhos quando recolhido; (c) FICHA JUDICIAL — override `.v2 .fjb-*` em componentes.css (vidro escuro + "Criar ficha" BotaoPrimario platina; verde só na tag status) para o FichaJudicialBox v1 reusado; guard de colisão ganhou allowlist `fjb-*` (override intencional de componente v1, não dá p/ renomear classe v1). AUDITORIA adversarial (6 finders Opus 4.8 + refutação): 3 confirmados corrigidos — cobranças no ctx viraram selo `.st.s-ok/s-at/s-er` (era texto colorido); Bolha ganhou estados 'Imagem/Vídeo indisponível' + 'Áudio não enviado' (v2 desenhava bolha vazia / player tocável p/ áudio falho); opt-out revalidado em `enviarEtapa` (Scripts) e `aoSubmeter` (Agendar) contra bloqueio por realtime com o modal aberto. Baixas tratadas: ícone de foco distinto do funil; `.wa-ctopo` bg .02 + `.sb` 10.5px; `.nl` #D9DCE1/pad 5px; guardas `if(!currentId)`/blob nulo na mídia; aviso de restrição com `!canalIndisponivel`. Real-env leitura-pura desta rodada bloqueada pelo restart do server (sessão limpa; não digito credenciais) — spotlight/recolher são código idêntico demo↔real e a ficha é CSS por especificidade, todos verificados no demo; o real já fora validado na sessão de implementação (371 conversas).

**Sessão feita (2026-07-28, commits "v2: hook do inbox" 91d2448 · "v2: whatsapp" d23efed · "v2: notificacao realtime" a47815b):** máquina inline (~600 l) extraída para src/v2/hooks/useInboxWhatsApp.ts + inboxWhatsApp.ts (funções puras com 11 testes: reconciliação lista×histórico, ciclo otimista cid/falha/retry, canal automático) — comportamento idêntico declarado (timeout 25s, retry sem assinatura/timeout, 6 guardas do marcar-lida, refs de canal manual). Página nos 3 painéis do mockup pg-wa (fila 296 · conversa · ctx 266) com paridade da lista acima; ADITIVOS declarados: mapMensagens ganhou origemBot ('bot' → selo "◈ Matheo" na bolha) e transcricao (metadados.transcricao → .transc; backend ainda não grava — UI pronta); OPT-OUT NOVO e inviolável (useBloqueiosOrg → banner + placeholder + textarea/mídia/rápidas/scripts desabilitados + bloco no ctx); respostas rápidas = PONTE DOS SCRIPTS (useScripts('whatsapp') reais → chips .rapidas → ScriptSequenceModal, nunca envio cego); AudioRecorderV2 = pele arec-* com lógica 1:1 (única divergência: sem spinner girante — dado é calmo); MediaComposer/ScriptSequenceModal/FichaJudicialBox reusados inteiros; AgendarMensagemModalV2 reusado; KanbanContatoBox recriado com deep-link /v2/kanban?oportunidade=; notificação realtime no shell (useNotificacaoInbound: canal v2-notif-<org> no MESMO feed postgres_changes, só INSERT entrada, suprime conversa aberta+focada, badge com pulso, Ver conversa → ?conversa=); "Simular resposta" fica como fallback dev. Real validado em LEITURA PURA (371 conversas; conversa aberta sem não-lidas — marcar-lida não dispara; janela 24h real do OFICIAL renderizada; rápidas com scripts reais; deep-link consumindo URL; ZERO envio).
...
**Sessão feita (2026-07-28, commit "v2: kanban"):** anatomia pg-kanban (.kb-cols/.kb-col/.kb-cab pt2+nome+contagem+soma/.kc com trilho/.fantasma) sobre o mecanismo de drag HTML5 da v1 (refs dragId/dragColId separados, otimista, auto-scroll 80px, locks) — só a pele mudou: tilt .kc.drag no card de origem + fantasma tracejado na coluna sob o cursor (no lugar do drop-hover). Página cheia: shell v2 ganhou `.pagina.cheia` por rota (kanban|whatsapp). NOVO do mockup/ordem (declarado): soma R$ compacta por coluna e subtítulo "N leads ativos · R$ X em potencial" (client-side por valorRelevante); estagnação "parado Xd" + trilho rubro client-side por movimentadoEm com LIMIAR_PARADO_DIAS=7 (não usa o motor SLA; chips de SLA continuam com SLA_OCULTO_NO_CARD da v1); trilho âmbar = alerta SLA lead_quente; badge "Não incomodar" no card via relacionamento_bloqueio (useBloqueiosOrg, precedente Contatos). FichaJudicialBox + RelacionamentoContatoBox REUSADOS inteiros no real (bytes v1; CSS namespaced próprio; Modal v1 z-index 1000 sobrepõe o ModalV2 90); demo mostra ficha semeada estática. Classes renomeadas por colisão com o app antigo (guard test): kb-det-*→kd-*, kb-hist*→kd-hist*, sevClass sla-*→s-*. Demo completo em memória (seed 6 colunas + 9 leads + SLA + fichas + eventos + opt-out); real validado em leitura absoluta (386 cards, zero mutação).
...
**Sessão feita (2026-07-28, commit "v2: integracoes"):** anatomia int-card do mockup pg-integracoes (int-l1/int-ic/int-n/int-s/int-l2/.mono webhook+Copiar) com a realidade vencendo a ficção — canais REAIS por card (nome, número mascarado, etiqueta de transporte Evolution·QR / Cloud API·Meta, BadgeStatus, chips Sessão/Entrega/Auto e alerta único em cascata, condições verbatim da v1). Sum-grid virou 4 Kpi (o 4º, maturação, só admin). Escopo da ordem = paridade total em QR/limite/saúde/ponte-maturação: WhatsAppConnect recriado (stages form|qr|connected, cancel no QR remove instância/libera vaga, renova 60s, poll 3s; demo = QR xadrez + conexão simulada 9s), DiagnosticoModal (12 pares + bloco entrega automática + últimos envios + notas em cascata), ConfigOrigemModal, confirmações byte-a-byte (Voltar do Adendo 4), Facebook completo (OAuth ?fb=, fbTokenBadge, 6 motivos). CORTES declarados: Maturação embutida → card-PONTE p/ /v2/maturacao (modais MatNovo/MatProxy/MatQr não duplicados); gestão pesada da Cloud API (cadastrar número, papel, CRUD/sincronizar templates) fora — card informativo com nota apontando a página antiga até sessão dedicada. Cores: laranja do wa-health mapeado em âmbar (v2 não tem laranja); toasts → aviso-inline. No real: URA em conflito com RMKT 5 (mesmo número), RMKT 4 com entrega Restrita — a cascata de alerta renderiza casos reais.
