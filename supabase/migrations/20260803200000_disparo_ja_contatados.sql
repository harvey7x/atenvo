-- ─────────────────────────────────────────────────────────────────────────────
-- DISPARO — "Já contatados": não perder de vista quem já recebeu um disparo.
--
-- Nada de balde pra onde a pessoa "vai": o histórico já mora em disparo_alvos
-- (status='enviado' + enviado_em + wamid). Aqui só EXPOMOS esse histórico:
--   1) disparo_elegiveis passa a marcar quem já recebeu (badge + filtro na tela);
--   2) disparo_contatados(p_org) = visão por PESSOA de quem já recebeu, para
--      re-disparar depois (os ids alimentam disparo_add_alvos, que re-checa
--      opt-out e WhatsApp no servidor — não exige estar no topo do funil).
-- "Recebeu" = tem alvo status='enviado' em QUALQUER campanha da org.
-- ─────────────────────────────────────────────────────────────────────────────

-- 1) elegíveis + carimbo de "já recebeu" (retorno muda ⇒ drop + recreate).
...
language sql stable security definer set search_path = public as $fn$
  ...
  -- último disparo ENVIADO de cada contato (nome da campanha + quando)
  disp as (
    ...
    where dc.organizacao_id = p_org and da.status = 'enviado'
...
-- 2) visão por PESSOA de quem já recebeu (aba "Já contatados").
-- Fonte da verdade = disparo_alvos status='enviado'. Um contato aparece uma vez,
-- com total de disparos, quando foi o último e em qual campanha. optout mostra
-- quem saiu (o re-disparo já barra no servidor, mas a tela avisa antes).
