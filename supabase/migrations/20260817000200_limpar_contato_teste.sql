-- ═══════════════════════════════════════════════════════════════════════════
-- LIMPEZA REUTILIZÁVEL DO NÚMERO DE TESTE — limpar_contato_teste(p_sufixo)
--
-- A rotina da limpeza manual (2026-08-15) virou RPC: localiza o(s) contato(s)
-- pelo SUFIXO do telefone (LIKE '%sufixo%' em contatos.telefone E em
-- contato_identidades.valor_normalizado — cobre o 9º dígito oscilante), aplica
-- a limpeza COMPLETA em transação (a função é atômica) e devolve o relatório
-- com contagens antes/depois por tabela.
--
-- SEGURANÇA:
--  * sufixo mínimo de 8 dígitos (nada de varrer meia base por acidente);
--  * aborta se casar MAIS de 2 contatos (com/sem 9 podem ter virado 2 registros;
--    3+ é sinal de sufixo ruim) — o relatório lista nome/telefone dos achados;
--  * PRESERVA audit_log e whatsapp_webhook_events (log bruto fica);
--  * service_role only (revoke de anon/authenticated).
--
-- Uso: select public.limpar_contato_teste('98872825');
--      ou pelo wrapper scripts/limpar-contato-teste 98872825
-- ═══════════════════════════════════════════════════════════════════════════
