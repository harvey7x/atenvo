-- IA SDR: debounce 8s→4s (resposta mais rápida) + cron 15s→10s. Re-aplica o trigger vindo de
-- 20260826140000 (retorno só via anúncio) com o intervalo reduzido.

-- quando ele volta a chamar PELO ANÚNCIO (Click-to-WhatsApp). Um "bom dia" espontâneo de um lead
-- fechado NÃO pode disparar o retorno — vai pro humano. O sinal de anúncio é a chave
-- `metadados.referral` que o webhook grava (ctwa_clid/source_type=ad), o MESMO gate que o fluxo
-- caf_emprestimo_v1 já usa (veioDeAnuncio). Sem referral na mensagem que chegou → sem retorno.
--
-- Nada aqui toca conversa ATIVA normal (o lead que responde a IA segue sendo atendido), nem o
-- fluxo, nem a retomada proativa (que é agendada, não vem de inbound).
...
begin
  ...
  if v_id is not null and v_status = 'ativa' then
    if not ( v_ad
      ...
      update public.ia_sessoes
        set ultima_msg_cliente_em = now(), processar_apos = now() + interval '4 seconds', atualizado_em = now()
        where id = v_id;
      return new;
    end if;
  end if;
  ...
  if v_id is not null then
    update public.ia_sessoes set
      ...
      processar_apos = now() + interval '4 seconds', ultima_msg_cliente_em = now(), atualizado_em = now()
    where id = v_id;
    insert into public.ia_eventos (sessao_id, conversa_id, organizacao_id, tipo, detalhe)
...
-- cron da ia-sdr: 15s → 10s (reduz a latência de captação da mensagem)
select cron.alter_job(23, schedule => '10 seconds');
