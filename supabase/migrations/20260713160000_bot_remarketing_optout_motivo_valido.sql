-- Fix²: motivo_perda tem CHECK (oportunidades_motivo_perda_chk) que só aceita
-- {sem_interesse, nao_respondeu, nao_elegivel, concorrente, dados_invalidos, outro}.
-- 'opt_out_remarketing' violava o check. Usamos 'sem_interesse' (encaixe semântico do opt-out;
-- não exige motivo_perda_desc, ao contrário de 'outro').
...
begin
  ...
  if v_col is not null then
    if v_optout then
      -- coluna PERDIDA exige motivo_perda válido (trigger + check). Preserva um motivo já existente.
      update public.oportunidades
        set coluna_id = v_col, motivo_perda = coalesce(motivo_perda, 'sem_interesse')
        where id = v_opp;
    else
