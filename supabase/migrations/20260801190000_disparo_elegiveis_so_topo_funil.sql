-- Disparo: público SÓ do topo do funil — coluna de ENTRADA (Lead Novo, pelo flag,
-- nunca pelo nome) + REMARKETING (pelo nome, mesmo critério do bot_remarketing_sync).
-- Decisão do dono (2026-08-02): DOCUMENTAÇÃO/CTT-ASSINAR são clientes com contrato
-- andando — template de retomada ("seu caso ficou pendente") confundiria quem já
-- fechou. Excluído na RPC (fonte), não só na tela, pra não entrar por acidente.
...
language sql stable security definer set search_path = public as $fn$
  with base as (
    ...
    where o.organizacao_id = p_org
      ...
      and (fc.entrada or fc.nome = 'REMARKETING')       -- só topo do funil
