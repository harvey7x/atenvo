-- Correção do merge_contatos/desfazer_merge: bot_conversa_estado tem PK = conversa_id
-- (não possui coluna "id"), então não pode entrar na lista genérica de re-pontagem por id.
-- Passa a ser re-pontado pela sua conversa (merge) e re-sincronizado na reversão (desfazer).
-- Sem mudança de comportamento nas demais tabelas. Nenhum merge é executado aqui.
