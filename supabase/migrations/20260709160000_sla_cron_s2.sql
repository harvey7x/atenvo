-- ============================================================================
-- SLA — S2: liga a automação. pg_cron 'sla-avaliar' a cada 1 min chamando
-- sla_avaliar(null) direto no banco (sem HTTP/secret; função pura). Re-carimba
-- ativo_desde=now() no instante da ativação. Endurece o motor para avaliar
-- APENAS orgs com sla_config ATIVA (opt-in) — evita backlog de outras orgs.
-- NÃO mexe em front, e-mail/push, bot B3, health check, GlobalWhatsAppAlert,
-- Relatórios, Ficha, Cobranças ou distribuição.
-- ============================================================================

create extension if not exists pg_cron;

-- Endurecimento: só avalia conversas/oportunidades de orgs com sla_config.ativo=true
-- (join, não left join). Corte de data e demais regras inalterados.
...
begin
  ...
  join public.sla_config sc on sc.organizacao_id = c.organizacao_id and sc.ativo     -- opt-in por org
  ...
  where c.arquivada_em is null and c.status in ('aberta','em_atendimento','pendente')
    ...
    and (sc.ativo_desde is null or c.criado_em >= sc.ativo_desde);   -- corte de data (conversas)
  ...
  join public.sla_config sc on sc.organizacao_id = o.organizacao_id and sc.ativo      -- opt-in por org
  where o.status = 'em_andamento' and (p_org is null or o.organizacao_id = p_org)
    and (sc.ativo_desde is null or o.entrada_em >= sc.ativo_desde);  -- corte de data (oportunidades)
...
-- Re-carimba a ativação AGORA (instante do S2) para todas as orgs com SLA ativo.
update public.sla_config set ativo_desde = now() where ativo;

-- Cron a cada 1 minuto (idempotente por nome). Chama a função pura direto no banco.
select cron.schedule('sla-avaliar', '* * * * *', $cron$ select public.sla_avaliar(null); $cron$);
