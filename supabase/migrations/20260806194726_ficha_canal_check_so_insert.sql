-- Ficha judicial: checagem de canal (canal_conversa_divergente) passa a valer SÓ no INSERT.
-- Motivo: canal_id da ficha é imutável, mas a conversa migra de canal (ex.: OFICIAL→LUIZA);
-- no UPDATE a checagem tornava a ficha permanentemente ineditável (caso ELBA 2026-08-06).
-- Base: fn_ficha_before de 20260731130000_sync_responsavel_fix_auth.sql (versão viva em prod), única mudança no bloco do canal.
