-- "Não mora no RS" como novo motivo de NÃO ELEGÍVEL (pedido do dono 17/09: medir quantos
-- leads chegam de fora do estado). Aditivo: só amplia o conjunto permitido; nenhum dado antigo tocado.
...
alter table public.oportunidades add constraint oportunidades_motivo_nao_elegivel_chk
  check (motivo_nao_elegivel is null or motivo_nao_elegivel in
    ('sem_beneficio_inss','muitos_processos','sem_interesse','sem_emprestimo_banco','nao_mora_rs'));
...
begin
  if p_motivo is not null and p_motivo not in
     ('sem_beneficio_inss','muitos_processos','sem_interesse','sem_emprestimo_banco','nao_mora_rs') then
...
-- 3) dashboard_trafego_periodo: contador ne_nao_mora_rs (senão a categoria some do breakdown por canal)
...
as $function$
  ...
  from (
    select
      ...
      count(*) filter (where o.status = 'perdido' and o.fechado_em >= p_ini and o.fechado_em < p_fim
        and coalesce(o.motivo_perda, '') = any (public.dashboard_motivos_descarte())
        and o.motivo_nao_elegivel = 'nao_mora_rs') as ne_nao_mora_rs,
      count(*) filter (where o.status = 'perdido' and o.fechado_em >= p_ini and o.fechado_em < p_fim
        and coalesce(o.motivo_perda, '') = any (public.dashboard_motivos_descarte())
