...
function enviadorEvolution(instancia: string): Enviador {
  return {
    ...
    // vCard TEM equivalente no Baileys (/message/sendContact) — o cliente recebe o mesmo cartão
    // nativo. `wuid` = só dígitos (é o que faz o WhatsApp reconhecer a conta e oferecer "Conversar").
    sendContato: (numero, nome, telefone) => evolution.sendContact(instancia, numero, [{
      fullName: nome, wuid: telefone.replace(/\D/g, ''), phoneNumber: `+${telefone.replace(/\D/g, '')}`,
    }]),
  };
}
